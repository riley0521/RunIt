...
class FirebaseRemoteRunDataSource(
    ...
    private suspend fun getRunDtoById(id: String): RunDtoV2? = withContext(dispatcherProvider.io) {
        val result = runCollection.document(id).get().await()?.toObject(RunDtoV2::class.java)
